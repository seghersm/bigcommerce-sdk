...
public class BigcommerceSdk {
  ...
  public CatalogSummary getCatalogSummary() {
    ...
    final CatalogSummaryResponse catalogSummaryResponse = get(webTarget,
        CatalogSummaryResponse.class);
  ...
  public ProductImageResponse createCategoryImage(final int categoryId, String imageUrl) {
    ...
    final WebTarget webTarget = baseWebTargetV3.path(CATALOG).path(CATEGORIES)
        .path(String.valueOf(categoryId)).path(IMAGE);
  ...
  public ProductImage updateProductImage(final ProductImage productImage) {
    ...
    final ProductImageResponse productImageResponse = put(webTarget, productImage,
        ProductImageResponse.class);
  ...
  public CustomFieldResponse createProductCustomField(final Integer productId,
  ...
  public List<Order> getOrders(final int page, final int limit, final DateTime earliestDate) {
    final WebTarget webTarget = baseWebTargetV2.path(ORDERS).queryParam(LIMIT, limit)
        .queryParam(PAGE, page)
  ...
  public OrderStatus getStatus(final com.bigcommerce.catalog.models.Status statusName) {
    ...
    return statusResponse.stream().filter(status -> status.getName().equals(statusName.toString()))
        .findFirst()
        .get();
  }
  ...
  public Variant updateVariant(final Variant variant) {
    final WebTarget webTarget = baseWebTargetV3.path(CATALOG).path(PRODUCTS)
        .path(String.valueOf(variant.getProductId())).path(VARIANTS)
        .path(String.valueOf(variant.getId()));
  ...
  public Brands getBrands(final int page, final int limit) {
    final WebTarget webTarget = baseWebTargetV3.path(CATALOG).path(BRANDS).queryParam(LIMIT, limit)
        .queryParam(PAGE,
            page);
  ...
  public Shipment createShipment(final ShipmentCreationRequest shipmentCreationRequest,
      final int orderId) {
    final WebTarget webTarget = baseWebTargetV2.path(ORDERS).path(String.valueOf(orderId))
        .path(SHIPMENTS);
  ...
  private Retryer<Response> buildResponseRetyer() {
    return RetryerBuilder.<Response>newBuilder().retryIfResult(this::shouldRetryResponse)
        ...
        .withStopStrategy(
            StopStrategies.stopAfterDelay(requestRetryTimeoutDuration, requestRetryTimeoutUnit))
        .build();
  }
  ...
  private <T> T handleResponse(final Response response, final Class<T> entityType,
    ...
    final List<Integer> expectedStatusCodes = Arrays.asList(expectedStatuses).stream()
    ...
    public long computeSleepTime(@SuppressWarnings("rawtypes") final Attempt failedAttempt) {
      if (failedAttempt.hasResult()) {
        if (failedAttempt.getResult() instanceof Response) {
          ...
          final String rateLimitTimeResetString = response
              .getHeaderString(RATE_LIMIT_TIME_RESET_HEADER);
          if (TOO_MANY_REQUESTS_STATUS_CODE == response.getStatus()
              && rateLimitTimeResetString != null) {
